...
import {Route, NotFound} from '../../';

describe('matchRoutes', function () {

  var routes = [
    <Route path='(/)' handler={<div name="root"/>}/>,
    <Route path='/cat/:id' handler={<div name="cat"/>}/>,
    <Route path='/mod/*' handler={<div name="mod"/>}/>,
    <Route path={/\/regex\/([a-zA-Z]*)$/} handler={<div name="regex"/>}/>,
    <Route path={/\/(.*?)\/(\d)\/([a-zA-Z]*)$/} handler={<div name="regexMatch"/>}
  ...
  it('matches a very custom url-pattern compiler config', function () {
    var route = <Route path="[http[s]!://][$sub_domain.]$domain.$toplevel-domain[/?]"
    ...
    assert.deepEqual(match.match, {
      sub_domain: 'www', domain: 'github', 'toplevel-domain': 'com',
